"""Behavioral scaffold tests for OpenModelFidelityPassport.
...
def test_allow_path_returns_digest() -> None:
    mech = OpenModelFidelityPassport()
    receipt = mech.evaluate(
        OpenModelFidelityPassportRequest(subject_id="a", payload={"x": 1}, budget=1.0)
    )
...
def test_refuse_missing_subject() -> None:
    mech = OpenModelFidelityPassport()
    receipt = mech.evaluate(OpenModelFidelityPassportRequest(subject_id="  ", payload={}, budget=1.0))
...
def test_refuse_non_positive_budget() -> None:
    mech = OpenModelFidelityPassport()
    receipt = mech.evaluate(OpenModelFidelityPassportRequest(subject_id="a", payload={}, budget=0.0))
    assert receipt.decision is Decision.REFUSE
...
def test_different_payloads_different_digests() -> None:
    mech = OpenModelFidelityPassport()
    a = mech.evaluate(OpenModelFidelityPassportRequest(subject_id="a", payload={"n": 1}, budget=1.0))
    b = mech.evaluate(OpenModelFidelityPassportRequest(subject_id="a", payload={"n": 2}, budget=1.0))
